    ...
    <div className={`min-h-screen relative p-4 overflow-hidden transition-all duration-1000 ${isLoaded ? 'animate-dashboard-entrance' : 'opacity-0 scale-95'} bg-white`}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className={`mb-3 relative transition-all duration-1050 ${isLoaded ? 'animate-slide-up' : 'opacity-0 translate-y-10'}`}>
          ...
          <h3 className="text-lg font-semibold mb-2 text-gray-700">Tra cứu </h3>
          <div className="grid grid-cols-1 gap-2">
            <Card
              className="p-4 cursor-pointer transition-all duration-200 hover:scale-105 hover:shadow-lg border-0 shadow-sm relative overflow-hidden bg-gradient-to-r from-blue-50 to-purple-50"
              onClick={() => onFeatureSelect('dictionary')}
            >
              {/* Decorative elements */}
              <div className="absolute top-1 right-2 text-md opacity-10">📚</div>
              <div className="absolute bottom-1 left-2 text-xs opacity-10">📖</div>

              {/* Top row - Icons */}
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className="p-2 bg-gradient-to-br from-blue-500 to-purple-500 rounded-2xl shadow-md">
                    <Languages className="w-5 h-5 text-white" />
                  </div>
                  <div className="p-2 bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl shadow-md">
                    <Volume2 className="w-5 h-5 text-white" />
                  </div>
                  <div className="p-2 bg-gradient-to-br from-pink-500 to-red-500 rounded-2xl shadow-md">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex gap-2 mt-1.5">
                      <span className="px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-600">🇺🇸 Tiếng Anh</span>
                      <span className="px-2 py-1 rounded-full text-xs bg-red-100 text-red-600">🇻🇳 Tiếng Việt</span>
                      <span className="px-2 py-1 rounded-full text-xs bg-purple-100 text-purple-600">🔊 Phiên âm</span>
                    </div>
                  </div>
                </div>
              </div>
