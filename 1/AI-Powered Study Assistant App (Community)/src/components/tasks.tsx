...
export function Tasks({ onBack }: TasksProps) {
      ...
      tasks.forEach(task => {
        if (!task.completed && task.dueDate <= tomorrow && task.dueDate >= now) {
          ...
          if (Notification.permission === 'granted') {
            new Notification(`Nhiệm vụ sắp đến hạn: ${task.title}`, {
              body: `Hạn chót: ${task.dueDate.toLocaleDateString('vi-VN')} ${task.dueDate.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}`,
              icon: '/favicon.ico'
            });
          }
        }
  ...
  const createTask = () => {
    ...
    toast.success("✅ Nhiệm vụ đã được tạo!", {
      description: `Nhiệm vụ "${task.title}" đã được thêm vào danh sách`,
      duration: 3000,
    });
  };
  ...
  const toggleTask = (id: string) => {
    ...
    if (!task.completed) {
      toast.success("🎉 Nhiệm vụ đã hoàn thành!", {
        description: `Chúc mừng! Bạn đã hoàn thành "${task.title}"`,
        duration: 3000,
      });
    } else {
      toast.info("↩️ Nhiệm vụ đã được đánh dấu chưa hoàn thành", {
        description: `"${task.title}" đã được khôi phục`,
        duration: 2000,
      });
    }
  };
  ...
  const deleteTask = (id: string) => {
    ...
    if (confirm('Bạn có chắc chắn muốn xóa nhiệm vụ này?')) {
      ...
      toast.success("🗑️ Nhiệm vụ đã được xóa", {
        description: `"${task.title}" đã được xóa khỏi danh sách`,
        duration: 2000,
      });
    }
  };
