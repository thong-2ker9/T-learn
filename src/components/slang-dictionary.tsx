...
import {ArrowLeft,Search,Heart,Menu,MessageCircle,Bookmark,Share2,Plus,Shuffle,Loader2} from "lucide-react";
...
export function SlangDictionary({
  ...
  useEffect(() => {
    const saved = localStorage.getItem('slangDictionary_customWords');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (customWords.length > 0) {
      localStorage.setItem('slangDictionary_customWords', JSON.stringify(customWords));
    }
  }, [customWords]);
